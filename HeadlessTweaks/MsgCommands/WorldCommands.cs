    ...
    partial class MessageCommands
    {
        public partial class Commands
        {
            ...
            public static void Users(UserMessages userMessages, Message msg, string[] args)
            {
                ...
                messages.Add(
                    $"{world.UserCount} users in the world \"{world.Name}\"{((world.UserCount != 0) ? ':' : null)}"
                );
            ...
            )
            {
                ...
                _ = userMessages.SendTextMessage(
                    $"Saved world \"{world.Name}\" in {timeElapsed.FormatAuto("0.##")}"
                );
            }
            ...
            [Command(
                ...
                usage: "[template name] [?SessionAccessLevel]"
            )]
            public static async Task StartWorldTemplate(
                UserMessages userMessages,
                Message msg,
                string[] args
            )
            {
                ...
                SessionAccessLevel sessionAccessLevel =
                    HeadlessTweaks.DefaultSessionAccessLevel.GetValue();
                ...
                if (
                    WorldPresets.Presets.FirstOrDefault(p =>
                        p.Name != null
                        && p.Name.Equals(
                ...
                {
                    _ = userMessages.SendTextMessage(
                        $"Unrecognized preset name \"{startInfo.LoadWorldPresetName}\""
                    );
                    return;
                }
            ...
            [Command(
                ...
                "World Management",
                PermissionLevel.Moderator,
                usage: "[world name] [?SessionAccessLevel]"
            )]
            ...
            )
            {
                ...
                if (!WorldRoster.TryGetValue(args[0], out string worldUrl))
                    return;
            ...
            [Command(
                ...
                "World Management",
                PermissionLevel.Moderator,
                usage: "[record url] [?SessionAccessLevel]"
            )]
            public static async Task StartWorldUrl(
                UserMessages userMessages,
                Message msg,
                string[] args
            )
            ...
            [Command(
                ...
                "World Management",
                PermissionLevel.Moderator,
                usage: "[?SessionAccessLevel]"
            )]
            public static async Task StartWorldOrb(
                UserMessages userMessages,
                Message msg,
                string[] args
            )
            ...
            public static async Task Role(UserMessages userMessages, Message msg, string[] args)
            {
                ...
                {
                    _ = userMessages.SendTextMessage(
                        $"User \"{userArg}\" not found in world \"{world.Name}\""
                    );
                    return;
                }

                string roleName = args[1];
                var role = world.Permissions.Roles.FirstOrDefault(x =>
                    x.RoleName.Value.Equals(roleName, StringComparison.InvariantCultureIgnoreCase)
                );
                ...
                _ = userMessages.SendTextMessage(
                    $"Set user \"{user.UserName}\" to \"{role.RoleName}\""
                );
            }
            ...
            public static async Task AddWorld(UserMessages userMessages, Message msg, string[] args)
            {
                ...
                {
                    // Ask the user to send a world orb
                    var recievedObject = await userMessages.RequestObjectMessage(
                        "No Uri provided, send a world orb"
                    );
            ...
            public static void RemoveWorld(UserMessages userMessages, Message msg, string[] args)
            {
                ...
                {
                    _ = userMessages.SendTextMessage(
                        $"World \"{worldName}\" doesn't exist in the roster"
                    );
                    return;
                }
            ...
            public static void ListRosterWorlds(
                UserMessages userMessages,
                Message msg,
                string[] args
            )
            ...
            [Command(
                "listWorlds",
                "List all worlds in the world roster and in the world presets",
                "World Management",
                PermissionLevel.Moderator
            )]
            ...
            )
            {
                ...
                var confirmMsg = await userMessages.RequestTextMessage(
                    $"Are you sure you want to close the world \"{world.Name}\"? (y/n)",
                    32
                );
                if (confirmMsg == null)
                {
                ...
                {
                    _ = userMessages.SendTextMessage(
                        $"Canceled, not closing world \"{world.Name}\""
                    );
                }
            }
            ...
            )
            {
                ...
                {
                    _ = userMessages.SendTextMessage(
                        "Usage: /setSessionAccessLevel [access level] [?hidden] [?world name...]"
                    );
                    return;
                }
                ...
                _ = userMessages.SendTextMessage(
                    $"Session access level set to {(hidden ? "Hidden, " : null)}{accessLevel} for \"{world.Name}\""
                );
            }
            ...
            )
            {
                ...
                {
                    _ = userMessages.SendTextMessage(
                        $"Invalid value for hidden argument: '{args[0]}'"
                    );
                    return;
                }
                ...
                {
                    _ = userMessages.SendTextMessage(
                        $"World {world.Name} is now hidden from listing"
                    );
                    return;
                }
            ...
            )
            {
                ...
                // Get the new name
                var newName = await userMessages.RequestTextMessage(
