...
namespace HeadlessTweaks
{
    partial class MessageCommands
    {
        ...
        public static async Task<string> TryGetUserId(
            ...
            bool onlyLookupContacts = false
        )
        {
            if (allowAnyUserId && IdUtil.GetOwnerType(value.ToUpper()) == OwnerType.User)
                return value;

            var contact = Engine.Current.Cloud.Contacts.FindContact(
                (Contact f) =>
                    f.ContactUsername.Equals(value, StringComparison.InvariantCultureIgnoreCase)
            );

            if (contact != null)
                return contact.ContactUserId;

            if (onlyLookupContacts)
                return null;
        ...
        private static World GetWorld(UserMessages userMessages, string worldName)
        {
            ...
            {
                world = worlds
                    .Where(w => w.RawName == worldName || w.SessionId == worldName)
                    .FirstOrDefault();
            }
        ...
        )
        {
            ...
            { // if no world name given, get the user's world
                var userWorlds = Engine.Current.WorldManager.Worlds.Where(w =>
                    w.GetUserByUserId(userId) != null
                );
                if (userWorlds.Any())
                {
                    world = userWorlds.FirstOrDefault(
                        (w) => w.GetUserByUserId(userId).IsPresentInWorld
                    );
                }
        ...
        )
        {
            ...
            var confirmMsg = await userMessages.RequestTextMessage(
                "Do you want to name this session? (y/n)",
                32
            );
            ...
            if (confirmText == "y" || confirmText == "yes")
            {
                ...
                else
                {
                    _ = userMessages.SendTextMessage(
                        "No was name entered, using the world's default name"
                    );
                }
            }
            ...
            var newWorld = new FrooxEngine.Headless.WorldHandler(
                handler.Engine,
                handler.Config,
                startInfo
            );
        ...
        private static async Task<Uri> ExtractOrbUrl(UserMessages userMessages, string objectUri)
        {
            ...
            await world.Coroutines.StartTask(async () =>
            {
                ...
                if (orb == null || orb.URL == null)
                {
                    ...
                    if (orb == null)
                        error = $"Object \"{slot.Name}\" is not a world orb";
        ...
        {
            _commandHandler ??= Traverse
                .CreateWithType("FrooxEngine.Headless.Program")
                ?.Field<FrooxEngine.Headless.CommandHandler>("commandHandler")
                ?.Value;
        ...
        public delegate void MessageCommandAction(
            UserMessages userMessages,
            Message msg,
            string[] args
        );
        ...
        class BatchMessageHelper(UserMessages userMessages, string prefix = null)
        {
            ...
            )
            {
                ...
                if (
                    !forceNewMessage
                    && Messages.Count != 0
                    && Messages.Last().Length + message.Length < 512
                )
                {
                    Messages[^1] += "\n" + message;
                    return;
